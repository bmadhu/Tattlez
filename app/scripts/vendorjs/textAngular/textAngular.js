...
(function(){ // encapsulate all variables so they don't become global vars
	...
	*/
	...
	if(ie > 8 || ie === undefined){
		...
		_addCSSRule(topsheet, '.ta-scroll-window.form-control', "height: 40px; overflow: hidden; font-family: inherit; font-size: 100%; position: relative; padding: 0;");
		...
		_addCSSRule(topsheet, '.ta-scroll-window .ta-bind', "height: 30px; min-height: 90px; padding: 6px 12px;");
	...
	textAngular.directive("textAngular", [
		...
		return {
			...
			link: function(scope, element, attrs, ngModel){
				...
				if(attrs.taDefaultWrap === undefined) attrs.taDefaultWrap = '';
				
				/* istanbul ignore next: ie specific test */
				if (attrs.taDefaultWrap === '') {
					_defaultVal = '';
					_defaultTest = (ie === undefined)? '<span><br></span>' : (ie >= 11)? '<p><br></p>' : (ie <= 8)? '<P>&nbsp;</P>' : '<p>&nbsp;</p>';
				}else{
